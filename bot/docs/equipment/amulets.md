# Amulets (Амулеты)

**Slot:** `amulet`
**Описание:** Ожерелье/амулет. Универсальный слот - может давать как атакующие, так и защитные статы. Единственный слот с passive_dps_bonus и широким набором элементального урона.

---

## Подтипы

| Подтип                  | Код              | Особенность                     |
|-------------------------|------------------|---------------------------------|
| Pendant (Кулон)         | `amu_pendant`   | Баланс атаки и защиты           |
| Talisman (Талисман)     | `amu_talisman`  | Элементальный фокус             |
| Locket (Медальон)       | `amu_locket`    | Защитный фокус                  |

---

## Доступные статы и диапазоны

### % Physical Damage (`pct_phys_dmg`)

| Tier | iLvl      | Min | Max  |
|------|-----------|-----|------|
| T5   | 1 - 19    | 3%  | 12%  |
| T4   | 20 - 39   | 3%  | 25%  |
| T3   | 40 - 59   | 3%  | 45%  |
| T2   | 60 - 79   | 3%  | 70%  |
| T1   | 80 - 100  | 3%  | 100% |

### Flat Fire Damage (`flat_fire_dmg`)

| Tier | iLvl      | Min | Max |
|------|-----------|-----|-----|
| T5   | 1 - 19    | 1   | 3   |
| T4   | 20 - 39   | 1   | 8   |
| T3   | 40 - 59   | 1   | 16  |
| T2   | 60 - 79   | 1   | 28  |
| T1   | 80 - 100  | 1   | 45  |

### Flat Cold Damage (`flat_cold_dmg`)

| Tier | iLvl      | Min | Max |
|------|-----------|-----|-----|
| T5   | 1 - 19    | 1   | 3   |
| T4   | 20 - 39   | 1   | 8   |
| T3   | 40 - 59   | 1   | 16  |
| T2   | 60 - 79   | 1   | 28  |
| T1   | 80 - 100  | 1   | 45  |

### Flat Lightning Damage (`flat_lightning_dmg`)

| Tier | iLvl      | Min | Max |
|------|-----------|-----|-----|
| T5   | 1 - 19    | 1   | 5   |
| T4   | 20 - 39   | 1   | 12  |
| T3   | 40 - 59   | 1   | 24  |
| T2   | 60 - 79   | 1   | 40  |
| T1   | 80 - 100  | 1   | 65  |

### Critical Strike Chance (`crit_chance`)

| Tier | iLvl      | Min  | Max  |
|------|-----------|------|------|
| T5   | 1 - 19    | 0.5% | 1.5% |
| T4   | 20 - 39   | 0.5% | 3%   |
| T3   | 40 - 59   | 0.5% | 5%   |
| T2   | 60 - 79   | 0.5% | 7%   |
| T1   | 80 - 100  | 0.5% | 10%  |

### Critical Strike Multiplier (`crit_multiplier`)

| Tier | iLvl      | Min  | Max  |
|------|-----------|------|------|
| T5   | 1 - 19    | 5%   | 12%  |
| T4   | 20 - 39   | 5%   | 22%  |
| T3   | 40 - 59   | 5%   | 35%  |
| T2   | 60 - 79   | 5%   | 52%  |
| T1   | 80 - 100  | 5%   | 72%  |

### Flat Life (`flat_hp`)

| Tier | iLvl      | Min | Max |
|------|-----------|-----|-----|
| T5   | 1 - 19    | 3   | 15  |
| T4   | 20 - 39   | 3   | 30  |
| T3   | 40 - 59   | 3   | 50  |
| T2   | 60 - 79   | 3   | 75  |
| T1   | 80 - 100  | 3   | 105 |

### % Increased Life (`pct_hp`)

| Tier | iLvl      | Min | Max  |
|------|-----------|-----|------|
| T5   | 1 - 19    | 2%  | 5%   |
| T4   | 20 - 39   | 2%  | 8%   |
| T3   | 40 - 59   | 2%  | 12%  |
| T2   | 60 - 79   | 2%  | 16%  |
| T1   | 80 - 100  | 2%  | 22%  |

### Gold Find (`gold_find`)

| Tier | iLvl      | Min | Max  |
|------|-----------|-----|------|
| T5   | 1 - 19    | 3%  | 10%  |
| T4   | 20 - 39   | 3%  | 20%  |
| T3   | 40 - 59   | 3%  | 35%  |
| T2   | 60 - 79   | 3%  | 55%  |
| T1   | 80 - 100  | 3%  | 80%  |

### Experience Bonus (`xp_bonus`)

| Tier | iLvl      | Min | Max |
|------|-----------|-----|-----|
| T5   | 1 - 19    | 1%  | 3%  |
| T4   | 20 - 39   | 1%  | 5%  |
| T3   | 40 - 59   | 1%  | 8%  |
| T2   | 60 - 79   | 1%  | 12% |
| T1   | 80 - 100  | 1%  | 16% |

### Life Regeneration (`life_regen`)

| Tier | iLvl      | Min  | Max   |
|------|-----------|------|-------|
| T5   | 1 - 19    | 0.5  | 2     |
| T4   | 20 - 39   | 0.5  | 5     |
| T3   | 40 - 59   | 0.5  | 10    |
| T2   | 60 - 79   | 0.5  | 18    |
| T1   | 80 - 100  | 0.5  | 28    |

### Passive DPS Bonus (`passive_dps_bonus`)

| Tier | iLvl      | Min | Max  |
|------|-----------|-----|------|
| T5   | 1 - 19    | 2%  | 8%   |
| T4   | 20 - 39   | 2%  | 15%  |
| T3   | 40 - 59   | 2%  | 28%  |
| T2   | 60 - 79   | 2%  | 45%  |
| T1   | 80 - 100  | 2%  | 65%  |

### Fire Resistance (`fire_res`)

| Tier | iLvl      | Min | Max  |
|------|-----------|-----|------|
| T5   | 1 - 19    | 3%  | 8%   |
| T4   | 20 - 39   | 3%  | 14%  |
| T3   | 40 - 59   | 3%  | 22%  |
| T2   | 60 - 79   | 3%  | 32%  |
| T1   | 80 - 100  | 3%  | 45%  |

### Cold Resistance (`cold_res`)

| Tier | iLvl      | Min | Max  |
|------|-----------|-----|------|
| T5   | 1 - 19    | 3%  | 8%   |
| T4   | 20 - 39   | 3%  | 14%  |
| T3   | 40 - 59   | 3%  | 22%  |
| T2   | 60 - 79   | 3%  | 32%  |
| T1   | 80 - 100  | 3%  | 45%  |

### Lightning Resistance (`lightning_res`)

| Tier | iLvl      | Min | Max  |
|------|-----------|-----|------|
| T5   | 1 - 19    | 3%  | 8%   |
| T4   | 20 - 39   | 3%  | 14%  |
| T3   | 40 - 59   | 3%  | 22%  |
| T2   | 60 - 79   | 3%  | 32%  |
| T1   | 80 - 100  | 3%  | 45%  |

### Physical Resistance (`phys_res`)

| Tier | iLvl      | Min | Max  |
|------|-----------|-----|------|
| T5   | 1 - 19    | 3%  | 8%   |
| T4   | 20 - 39   | 3%  | 14%  |
| T3   | 40 - 59   | 3%  | 22%  |
| T2   | 60 - 79   | 3%  | 32%  |
| T1   | 80 - 100  | 3%  | 45%  |

> Амулет может нести резисты вместе с атакующими статами — уникально. Locket (защитный фокус) идеален для phys_res + flat_hp комбинации.

---

## Пример генерации

**iLvl 15, Common Pendant:**
```
Copper Pendant (Amulet)
Item Level: 15 | Required Level: 11
Rarity: Common

+ 12 Life
+ 7% Gold Find
```

**iLvl 85, Legendary Talisman:**
```
Dragon's Eye (Amulet - Talisman)
Item Level: 85 | Required Level: 63
Rarity: Legendary

+ 88 Life
+ 85% Physical Damage
+ 40 Fire Damage
+ 8% Critical Strike Chance
+ 60% Critical Strike Multiplier
+ 55% Passive DPS Bonus
```
