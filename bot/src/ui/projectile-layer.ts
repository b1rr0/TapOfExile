...
export class ProjectileLayer {
  ...
  launch(
    ...
    this._projectiles.push({
      ...
      frameElapsed: 0,
      onImpact,
      playOnce: renderType !== "projectile",
    });
  }
    ...
    for (let i = 0; i < this._projectiles.length; i++) {
      ...
      if (p.frameElapsed >= frameInterval) {
        ...
        if (p.playOnce) {
          ...
          if (p.frameIndex < sprite.frames.length - 1) {
            p.frameIndex++;
          }
        } else {
      ...
      if (p.playOnce) {
        // Ends when last frame reached AND elapsed >= duration
        const onLastFrame = p.frameIndex >= sprite.frames.length - 1;
