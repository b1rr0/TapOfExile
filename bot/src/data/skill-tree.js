...
import {
  MINOR_POOL, NOTABLE_POOL, KEYSTONE_POOL, CLASS_SKILLS, NodeDef,
...
const CX = 800;
const CY = 800;
const CLASS_IDS = ["samurai", "warrior", "mage", "archer"];
const CLASS_ANGLE = { samurai: 0, warrior: Math.PI / 2, mage: Math.PI, archer: (3 * Math.PI) / 2 };

const START_RADIUS = 210;
export const EMBLEM_RADIUS = 100;
export const MAX_CLASS_SKILLS = 6;
...
export class SkillNode {
  /**
   * @param {number}        id             — numeric index (used by scene for allocated set & edges)
   * @param {string}        nodeId         — human-readable unique id, e.g. "samurai-start", "mage-cs-5"
   * @param {string}        type           — "start" | "classSkill" | "minor" | "notable" | "keystone"
   * @param {string}        classAffinity  — owning class id
   * @param {number}        x
   * @param {number}        y
   * @param {NodeDef|object} data          — NodeDef instance or legacy { label, name?, stat?, value? }
   */
...
export class SkillTreeBuilder {
  ...
  nextMinor()    { return MINOR_POOL[this._minorIdx++ % MINOR_POOL.length]; }
  nextNotable()  { return NOTABLE_POOL[this._notableIdx++ % NOTABLE_POOL.length]; }
  nextKeystone() { return KEYSTONE_POOL[this._keystoneIdx++ % KEYSTONE_POOL.length]; }
...
export function buildSkillTree() {
  ...
  const START_OFFSET_R = EMBLEM_RADIUS;
