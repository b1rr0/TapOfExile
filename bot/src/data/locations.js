/**
 * Location catalog — 5 acts × 10 locations each (50 total).
 *
 ...
 */
const REQUIREMENT_PATTERN = [0, 1, 2, 3, 4, 5, 6, 7, 3, 5];
 ...
 */
export function getBackgroundForLocation(location) {
 ...
 * @returns {{ icon: string, name: string, description: string, type: string }[]}
 */
export function getActModifiers(actNumber) {
 ...
 */
function buildActLocations(actNumber) {
...
export const ACT_DEFINITIONS = ACT_NAMES.map((name, i) => ({
  act: i + 1,
  name,
...
export const TOTAL_ACTS = ACT_DEFINITIONS.length;

/** Flat array of all 50 locations. */
export const ALL_LOCATIONS = ACT_DEFINITIONS.flatMap((a) => a.locations);
 ...
 * @param {number} actNumber — 1-5
 * @returns {Object[]}
 */
export function getLocationsForAct(actNumber) {
 ...
 * @returns {string[]}
 */
export function getMainChainIds(actNumber) {
 ...
 * @returns {boolean}
 */
export function isActComplete(actNumber, completedIds) {
 ...
 * @returns {number}
 */
export function getHighestUnlockedAct(completedIds) {
 ...
 * @returns {string}
 */
export function getActGateLocationId(actNumber) {
 ...
 * @returns {string}
 */
export function getActName(actNumber) {
  const def = ACT_DEFINITIONS.find((a) => a.act === actNumber);
