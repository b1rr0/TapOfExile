...
export function getBossKeyTierDef(tier) {
 ...
 *  bossType    — тип монстра (из monsters.js)
...
export function getWavesForTier(tier) {
 ...
 */
export function getBossMapWaves(bossId) {
 ...
 */
export function createMapKeyItem(tier) {
 ...
 * @param {string} bossId — id from BOSS_MAPS
 * @param {number} bossKeyTier — 1-3 (Standard / Empowered / Mythic)
 */
export function createBossMapKeyItem(bossId, bossKeyTier = 1) {
  const bossDef = getBossDef(bossId);
 ...
 * @param {number} tier — completed map tier (1-10)
 * @param {boolean} isBossMap
 * @param {string|null} direction — bossId for direction-based drops
 * @returns {Object[]} — array of map key items to add to bag
 */
export function rollMapDrops(tier, isBossMap = false, direction = null) {
  const drops = [];
  ...
  if (isBossMap) {
    ...
    if (Math.random() < S.boss.bossKeyChance) {
      ...
      if (direction) {
        drops.push(createBossMapKeyItem(direction, bkt));
      } else {
        const randomBoss = BOSS_MAPS[Math.floor(Math.random() * BOSS_MAPS.length)];
        drops.push(createBossMapKeyItem(randomBoss.id, bkt));
      }
    }
    return drops;
  }
 ...
 * @param {string[]} completedLocationIds
 * @returns {boolean}
 */
export function isEndgameUnlocked(completedLocationIds) {
