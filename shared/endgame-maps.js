...
exports.MAP_KEY_TYPES = exports.BOSS_MAPS = exports.MAP_TIERS = exports.MAX_TIER = exports.BOSS_KEY_TIERS = exports.DROP_SETTINGS = void 0;
exports.getBossKeyTierDef = getBossKeyTierDef;
exports.pickBossKeyTier = pickBossKeyTier;
exports.getTierDef = getTierDef;
exports.getBossDef = getBossDef;
exports.getWavesForTier = getWavesForTier;
exports.getBossMapWaves = getBossMapWaves;
exports.tierQuality = tierQuality;
exports.createMapKeyData = createMapKeyData;
exports.createBossKeyData = createBossKeyData;
exports.rollMapDrops = rollMapDrops;
exports.DROP_SETTINGS = {
    regular: {
        sameTierChance: 0.60,
        tierUpChance: 0.20,
        bossKeyChance: 0.05,
...
function getBossKeyTierDef(tier) {
    return exports.BOSS_KEY_TIERS[Math.max(0, Math.min(tier - 1, exports.BOSS_KEY_TIERS.length - 1))];
}
...
function getTierDef(tier) {
    return exports.MAP_TIERS[Math.max(0, Math.min(tier - 1, exports.MAP_TIERS.length - 1))];
}
exports.BOSS_MAPS = [
    {
...
function getBossDef(bossId) {
    return exports.BOSS_MAPS.find(b => b.id === bossId) || null;
}
const MAP_WAVE_TEMPLATES = [
    [
...
function createMapKeyData(tier) {
    return {
        id: `map_t${tier}_${uid()}`,
        name: `Tier ${tier} Map`,
        type: exports.MAP_KEY_TYPES.regular,
        tier,
...
function createBossKeyData(bossId, bossKeyTier) {
    const bossDef = getBossDef(bossId);
    if (!bossDef)
        return null;
    ...
    return {
        ...
        name: bossDef.name,
        type: exports.MAP_KEY_TYPES.boss,
        bossId,
        bossKeyTier,
        quality: bkt.quality,
        level: bossKeyTier,
...
function rollMapDrops(tier, isBossMap, direction, createMapKey, createBossKey) {
    const drops = [];
    const S = exports.DROP_SETTINGS;
    if (isBossMap) {
        const range = S.boss.guaranteedTierMax - S.boss.guaranteedTierMin + 1;
        const dropTier = Math.min(exports.MAX_TIER, Math.floor(Math.random() * range) + S.boss.guaranteedTierMin);
        drops.push(createMapKey(dropTier));
        if (Math.random() < S.boss.bonusKeyChance) {
            drops.push(createMapKey(Math.min(exports.MAX_TIER, dropTier + 1)));
        }
        if (Math.random() < S.boss.bossKeyChance) {
            const bkt = pickBossKeyTier(tier, true);
            const bossId = direction || exports.BOSS_MAPS[Math.floor(Math.random() * exports.BOSS_MAPS.length)].id;
    ...
    if (tier < exports.MAX_TIER && Math.random() < S.regular.tierUpChance) {
        drops.push(createMapKey(Math.min(exports.MAX_TIER, tier + 1)));
    }
...
//# sourceMappingURL=endgame-maps.js.map